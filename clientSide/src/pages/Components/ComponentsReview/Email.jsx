import { faBan, faBars, faCaretDown } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import PropTypes from 'prop-types';
import { useState } from 'react';
import React from 'react';

const Email = ({ state, displayMode, component, nextComponent }) => {
  const { bgColor, buttonColor, textColor, logo, language } = state;
  const [openEmail, setOpenEmail] = useState(true);
  const [emailInput, setEmailInput] = useState('');
  const handleEmailInputChange = (e) => {
    setEmailInput(e.target.value);
  };

  const textDirection = language === 'ar' ? 'rtl' : 'ltr';
  const baseURL = `https://pro-1-hk8q.onrender.com`;

  const truncateText = (text, maxLength) => {
    if (text.length > maxLength) {
      return text.slice(0, maxLength) + '...';
    }
    return text;
  };

  return (
    <div
      className={`bg-gray-100 flex flex-col gap-[20px] items-center justify-center border-gray-100 overflow-y rounded-xl shadow-xl ${
        displayMode === 'mobile'
          ? 'w-[450px] h-[750px] ml-[10px]'
          : 'w-[520px] h-[750px]'
      }`}
      dir={textDirection}
    >
      <div
        className={`top-0 left-0 right-0 w-full bg-gray-800 p-2 rounded-t-xl flex items-center justify-between`}
      >
        <div className="flex gap-2">
          <span className="bg-green-500 w-3 h-3 rounded-full cursor-pointer" />
          <span className="bg-yellow-500 w-3 h-3 rounded-full cursor-pointer" />
          <span className="bg-red-500 w-3 h-3 rounded-full cursor-pointer" />
        </div>
      </div>

      <div
        className={`bg-white border-[2px]  border-gray-100 shadow-lg flex flex-col gap-[20px] items-center justify-center  rounded-xl mb-[3px] ${
          displayMode === 'mobile'
            ? 'w-[400px] h-[670px]'
            : 'w-[500px] h-[680px]'
        }`}
      >
        {openEmail && (
          <div
            className="relative flex flex-col items-center justify-start  ml-[90px] transition-all ease-in-out duration-500"
            style={{
              width: displayMode === 'mobile' ? '400px' : '500px',
              height: displayMode === 'mobile' ? '600px' : '400px',
              marginTop: displayMode === 'mobile' ? '270px' : '250px',
              borderRadius: '15px',
            }}
          >
            <div className="flex flex-col items-center justify-center w-full relative">
              <FontAwesomeIcon
                icon={faCaretDown}
                className={`absolute w-[19px] h-[19px] p-[17px] cursor-pointer mb-[200px] ${
                  displayMode === 'mobile'
                    ? 'ml-[240px] mb-[300px]'
                    : 'ml-[360px] mb-[300px]'
                }`}
                onClick={() => setOpenEmail(false)}
                style={{ color: 'black', transition: 'color 0.3s' }}
              />
              <div
                className={`bg-white p-8  rounded-2xl shadow-2xl border-gray-200 border-[1px] w-full max-w-[350px] ${
                  displayMode === 'mobile'
                    ? 'p-6 mr-[36px] mt-[26px]'
                    : 'p-8 ml-[58px] mt-[40px]'
                }`}
                style={{ background: bgColor }}
              >
                <div className="w-full h-[60px] flex items-center justify-center mb-[20px]">
                  {logo && (
                    <img alt="" src={logo} className="w-[40px] h-[40px]" />
                  )}
                </div>
                <div className="flex flex-col items-center gap-[20px] max-h-[170px] overflow-y-auto overflow-x-hidden p-2  mt-[30px]">
                  <h1
                    className="text-[14px] text-gray-800  p-[10px]  font-semibold font-montserrat"
                    style={{ color: textColor }}
                  >
                    {component.question}
                  </h1>
                  <input
                    placeholder="Please type here..."
                    className={` transition duration-200 ease-in-out focus:outline-none focus:ring-1 focus:ring-gray-200 hover:border-gray-200 border-[2px]  shadow-lg text-[14px] border-gray-300 py-[8px]  hover:border-[3px] ${displayMode === 'mobile' ? 'w-[270px] p-[14px]' : 'w-[240px] p-[17px]'}`}
                    value={emailInput}
                    onChange={handleEmailInputChange}
                  />
                </div>
                <div className="flex items-center gap-[20px] justify-center   mt-6">
                  {language === 'en' && (
                    <button
                      className="underline text-[14px]  text-gray-700 hover:text-gray-900 transition-all"
                      onClick={nextComponent}
                    >
                      Skip
                    </button>
                  )}
                  {language === 'fr' && (
                    <button
                      className="underline text-[14px]  text-gray-700 hover:text-gray-900 transition-all"
                      onClick={nextComponent}
                    >
                      Passer
                    </button>
                  )}
                  {language === 'ar' && (
                    <button
                      className="underline text-[14px]  text-gray-700 hover:text-gray-900 transition-all"
                      onClick={nextComponent}
                    >
                      تجاوز
                    </button>
                  )}
                  {language === 'en' && (
                    <button
                      className={`border-2 border-transparent   rounded-xl text-[13px] py-1 px-4 w-30 font-bold bg-gradient-to-r from-blue-600 to-blue-800 text-white transition-all duration-300 ease-in-out hover:shadow-lg hover:scale-105 hover:bg-gradient-to-l focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 }`}
                      onClick={nextComponent}
                      style={{
                        background: !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(
                          emailInput
                        )
                          ? 'gray'
                          : buttonColor,
                      }}
                      disabled={!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput)}
                    >
                      Next{' '}
                      {!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput) && (
                        <FontAwesomeIcon
                          icon={faBan}
                          className="ml-[10px] text-red-700"
                        />
                      )}
                    </button>
                  )}
                  {language === 'fr' && (
                    <button
                      className={`border-2 border-transparent  rounded-xl text-[13px] py-1 px-4 w-30 font-bold bg-gradient-to-r from-blue-600 to-blue-800 text-white transition-all duration-300 ease-in-out hover:shadow-lg hover:scale-105 hover:bg-gradient-to-l focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 }`}
                      onClick={nextComponent}
                      style={{
                        background: !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(
                          emailInput
                        )
                          ? 'gray'
                          : buttonColor,
                      }}
                      disabled={!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput)}
                    >
                      Suivant{' '}
                      {!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput) && (
                        <FontAwesomeIcon
                          icon={faBan}
                          className="ml-[10px] text-red-700"
                        />
                      )}
                    </button>
                  )}
                  {language === 'ar' && (
                    <button
                      className={`border-2 border-transparent rounded-xl text-[13px] py-1 px-4 w-30 font-bold bg-gradient-to-r from-blue-600 to-blue-800 text-white transition-all duration-300 ease-in-out hover:shadow-lg hover:scale-105 hover:bg-gradient-to-l focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 }`}
                      onClick={nextComponent}
                      style={{
                        background: !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(
                          emailInput
                        )
                          ? 'gray'
                          : buttonColor,
                      }}
                      disabled={!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput)}
                    >
                      التالي{' '}
                      {!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailInput) && (
                        <FontAwesomeIcon
                          icon={faBan}
                          className="ml-[10px] text-red-700"
                        />
                      )}
                    </button>
                  )}
                </div>
              </div>
            </div>
          </div>
        )}
        {!openEmail && (
          <div
            className="relative flex flex-col items-center justify-center bg-white border-[2px] border-gray-100 rounded-xl"
            style={{
              width: displayMode === 'mobile' ? '330px' : '350px',
              height: displayMode === 'mobile' ? '40px' : '40px',
              top: displayMode === 'mobile' ? '315px' : '320px',
              marginLeft: displayMode === 'mobile' ? '70px' : '150px',
            }}
          >
            <FontAwesomeIcon
              icon={faCaretDown}
              className={`w-[19px] h-[19px] cursor-pointer text-gray-800 ${displayMode === 'mobile' ? 'ml-[300px]' : 'ml-[320px]'}`}
              onClick={() => setOpenEmail(true)}
            />
            <h1 className=" absolute text-[14px] text-gray-800 p-[20px] font-bold ">
              {truncateText(component.question, 20)}
            </h1>
          </div>
        )}
      </div>
    </div>
  );
};

Email.propTypes = {
  state: PropTypes.shape({
    bgColor: PropTypes.string,
    buttonColor: PropTypes.string,
    textColor: PropTypes.string,
    logo: PropTypes.string,
    language: PropTypes.oneOf(['en', 'fr', 'ar']),
  }).isRequired,
  displayMode: PropTypes.oneOf(['mobile', 'desktop']).isRequired,
  component: PropTypes.shape({
    question: PropTypes.string.isRequired,
    image: PropTypes.string,
    lowScoreTitle: PropTypes.string,
    highScoreTitle: PropTypes.string,
  }).isRequired,
  nextComponent: PropTypes.func.isRequired,
};
export default Email;
